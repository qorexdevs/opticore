...
class TestOIConcentration:
    def _chain(self):
        # call+put OI per strike: 100->700, 105->200, 95->100 (total 1000)
        rows = [
        ...
        return pd.DataFrame(
            [
                ...
                for e, k, kind, oi in rows
            ]
        )

    def test_shares_and_hhi(self):
        r = oc.oi_concentration(self._chain()).iloc[0]
        assert r["n_strikes"] == 3
        assert r["total_oi"] == pytest.approx(1000.0)
    ...
    def test_single_strike_hhi_is_one(self):
        ...
        r = oc.oi_concentration(chain).iloc[0]
        assert r["n_strikes"] == 1
        assert r["top_share"] == pytest.approx(1.0)
        assert r["hhi"] == pytest.approx(1.0)

    def test_one_row_per_expiry_sorted(self):
        out = oc.oi_concentration(_synthetic_chain(expiry_days=(120, 30, 60)))
        assert len(out) == 3
        assert list(out["expiry"]) == sorted(out["expiry"])

    def test_no_open_interest_column_returns_empty(self):
        chain = _synthetic_chain(expiry_days=(30,)).drop(columns=["open_interest"])
        out = oc.oi_concentration(chain)
        assert out.empty
...
class TestVegaConcentration:
    def _enriched(self, **kw):
        return oc.enrich(_synthetic_chain(**kw), rate=0.05)

    def test_returns_expected_columns(self):
        out = oc.vega_concentration(self._enriched(expiry_days=(30,)))
        for col in (
            "expiry",
            "underlying_price",
            "n_strikes",
    ...
    def test_one_row_per_expiry_sorted(self):
        out = oc.vega_concentration(self._enriched(expiry_days=(120, 30, 60)))
        assert len(out) == 3
        assert list(out["expiry"]) == sorted(out["expiry"])

    def test_shares_bounded_and_ordered(self):
        r = oc.vega_concentration(self._enriched(expiry_days=(30,), n_strikes=11)).iloc[0]
        assert 0.0 < r["top_share"] <= r["top3_share"] <= 1.0
        assert r["gross_vex"] > 0
    ...
    def test_top_strike_near_atm(self):
        # vega peaks at-the-money, so the heaviest strike sits at spot
        r = oc.vega_concentration(
            self._enriched(underlying=100.0, expiry_days=(30,), n_strikes=11)
        ).iloc[0]
        assert r["top_strike"] == pytest.approx(100.0)
    ...
    def test_single_strike_hhi_is_one(self):
        chain = self._enriched(underlying=100.0, expiry_days=(30,), n_strikes=11)
        chain = chain[chain["strike"] == 100.0]
        r = oc.vega_concentration(chain).iloc[0]
        assert r["n_strikes"] == 1
        assert r["top_share"] == pytest.approx(1.0)
        assert r["hhi"] == pytest.approx(1.0)

    def test_no_vega_column_returns_empty(self):
        out = oc.vega_concentration(_synthetic_chain(expiry_days=(30,)))
        assert out.empty
        assert "hhi" in out.columns

    def test_in_public_api(self):
        assert hasattr(oc, "vega_concentration")
        assert callable(oc.vega_concentration)
...
class TestThetaConcentration:
    def _enriched(self, **kw):
        return oc.enrich(_synthetic_chain(**kw), rate=0.05)

    def test_returns_expected_columns(self):
        out = oc.theta_concentration(self._enriched(expiry_days=(30,)))
        for col in (
            "expiry",
            "underlying_price",
            "n_strikes",
    ...
    def test_one_row_per_expiry_sorted(self):
        out = oc.theta_concentration(self._enriched(expiry_days=(120, 30, 60)))
        assert len(out) == 3
        assert list(out["expiry"]) == sorted(out["expiry"])

    def test_shares_bounded_and_ordered(self):
        r = oc.theta_concentration(self._enriched(expiry_days=(30,), n_strikes=11)).iloc[0]
        assert 0.0 < r["top_share"] <= r["top3_share"] <= 1.0
        assert r["gross_tex"] > 0
    ...
    def test_top_strike_near_atm(self):
        # theta peaks at-the-money, so the heaviest strike sits at spot
        r = oc.theta_concentration(
            self._enriched(underlying=100.0, expiry_days=(30,), n_strikes=11)
        ).iloc[0]
        assert r["top_strike"] == pytest.approx(100.0)
    ...
    def test_single_strike_hhi_is_one(self):
        chain = self._enriched(underlying=100.0, expiry_days=(30,), n_strikes=11)
        chain = chain[chain["strike"] == 100.0]
        r = oc.theta_concentration(chain).iloc[0]
        assert r["n_strikes"] == 1
        assert r["top_share"] == pytest.approx(1.0)
        assert r["hhi"] == pytest.approx(1.0)

    def test_no_theta_column_returns_empty(self):
        out = oc.theta_concentration(_synthetic_chain(expiry_days=(30,)))
        assert out.empty
        assert "hhi" in out.columns

    def test_in_public_api(self):
        assert hasattr(oc, "theta_concentration")
        assert callable(oc.theta_concentration)
